...
#include "Network/ice_messages.hpp"
...
namespace Stand
{
	...
	// Contains the full dispatch logic (std::string, fmt::format, Codename,
	// etc.), so it lives outside any __try. Returns false wherever the
	// original code used to `return;` early from within the __try (which
	// skipped both the call into the game's original function and the
	// packet_src reset), true otherwise.
	[[nodiscard]] static bool dispatchEventBody(rage::netConnectionManager::Channel* channel, rage::netConnectionManager* con_mgr, rage::netEvent* event)
	{
...
#define STACK_ALLOC_NO_CTOR(T, name) char name ## _scrap [sizeof(T)]; T& name = *reinterpret_cast<T*>(name ## _scrap);
	...
	void __fastcall rage_netConnectionManager_Channel_DispatchEvent(rage::netConnectionManager::Channel* channel, rage::netConnectionManager* con_mgr, rage::netEvent* event)
	{
		...
		__try
		{
			should_continue = dispatchEventBody(channel, con_mgr, event);
		}
		__EXCEPTIONAL()
		{
		}
