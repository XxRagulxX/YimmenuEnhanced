...
namespace Stand
{
	...
	[[nodiscard]] static std::vector<CommandListActionItemData> CommandTimecycleMod_getOptions()
	{
		all = {
			"AirRaceBoost01",
			"AirRaceBoost02",
			...
			"LightPollutionHills",
			"lightpolution",
			soup::ObfusString("LIGHTSreduceFALLOFF").str(),
		...
		options.emplace_back(CommandListActionItemData{ 0, LOC("OVRDOFF"), { CMDNAME("off") } });
		long long i = 0;
		for (const auto& shader : all)
		{
			CommandName command_name = utf8ToCmdName(shader);
			StringUtils::simplify(command_name);
			options.emplace_back(CommandListActionItemData{ ++i, LIT(shader), std::vector<CommandName>(1, std::move(command_name)) });
		}
		return options;
	}

	CommandTimecycleMod::CommandTimecycleMod(CommandList* const parent)
		: CommandListSelect(parent, LOC("SHDR"), CMDNAMES("shader"), NOLABEL, CommandTimecycleMod_getOptions(), 0, CMDFLAGS_LIST_SELECT, CMDFLAG_FEATURELIST_FINISHLIST)
	{
	}

	void CommandTimecycleMod::onChange(Click& click, long long prev_value)
	{
		if (value != 0)
		{
			...
			registerScriptTickEventHandler(click, [this, value]
			{
				if (this->value == value)
				{
					if (!shader_overriden)
					{
						GRAPHICS::SET_TIMECYCLE_MODIFIER(all[value - 1].c_str());
					}
					return true;
				}
				if (this->value == 0)
