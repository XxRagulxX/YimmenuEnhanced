...
namespace Stand
{
	CommandLuaTextsliderStateful::CommandLuaTextsliderStateful(CommandList* const parent, Label&& menu_name, std::vector<CommandName>&& command_names, Label&& help_text, CommandLuaScript* const script, std::vector<CommandTextsliderData>&& options, const int on_click_ref)
	...
	void CommandLuaTextsliderStateful::onClick(Click& click)
	{
		if (hasOptions())
		{
