...
namespace rage
{
	...
	public:
		...
		[[nodiscard]] CPhysical* getPhysical() const noexcept
		{
			...
			{
				const auto col3 = reinterpret_cast<const uint32_t*>(&m_Matrix.m_col3);
				const auto col2 = reinterpret_cast<const uint32_t*>(&m_Matrix.m_col2);

				const uint64_t address = uint64_t(col3[3]) | (uint64_t(col2[3]) << 32);

				return reinterpret_cast<CPhysical*>(address);
			}
