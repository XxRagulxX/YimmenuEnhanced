...
namespace YimMenu
{
	...
	std::shared_ptr<NodeHooks::Hook> NodeHooks::TryGetHook(CProjectBaseSyncDataNode* node)
	{
		...
		if (auto it = m_Hooks.find(node_id);
		    it != m_Hooks.end() && it->second->m_Enabled && it->second->m_InScopeCallback(object, it->second->m_PerPlayer ? GetSyncingPlayer() : nullptr))
			return it->second;
	...
	bool NodeHooks::ShouldSkipNodeCacheImpl(CProjectBaseSyncDataNode* node)
	{
		if (auto hook = TryGetHook(node))
			return hook->m_PerPlayer && GetSyncingObject();
